...
namespace NetWatch {
 class Fixture : IDisposable {
  ...
  public int Port;public volatile bool Respond=true;public volatile bool Reset=false;public volatile bool PortDown=false;public volatile bool Discontinuity=false;public volatile bool Errors=false;public volatile bool HighCpu=false;public volatile bool Wide=true;
 ...
 class Tests {
  ...
  class DashboardResult {public List<Alert> Alerts{get;set;}}
  ...
  public static int Main(string[] args){try{
   ...
   using(var fixture=new Fixture()){
    ...
    using(var restored=new Engine(store,false)){Check(restored.History(id).Count>=8&&Current(restored,id).Snmp,"restart restores configuration, state and history");restored.Delete(id);Check(File.Exists(Path.Combine(root,id+".json")),"removal retains historical file for recovery");}
   }
